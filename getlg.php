...
// Demo: http://linkedopendatang.com/entity/localgov/getlg.php?f=xml
...
$path = "select distinct ?s1 as ?lgURI str(?s4) as ?lgName  str(?s5) as ?lgDesc ?s6 as ?lgState " ;
$path .= "where  { " ;
$path .= "?s1 a <http://dbpedia.org/ontology/Place> . " ;
$path .= "?s1 <http://dbpedia.org/ontology/type> ?s3 . " ;
$path .= "?s1 rdfs:label ?s4 .  " ;
$path .= "?s1 rdfs:comment ?s5 . " ;
$path .= "?s1 <http://dbpedia.org/property/subdivisionName> ?s6 . " ;
$path .= "filter ( ?s3 in ( <http://dbpedia.org/resource/Local_government_areas_of_Nigeria> ) )  " ;
